...
try:
    payload_list = []
    with open("Options/sql_payloads.txt", "r", encoding="utf-8") as file:
    ...
    if mod == 1:
        default_url = f"http://{target_ip}/search?id="
        target_url_input = input(f"{yellow}[?] Please enter the target URL in full format 'e.g., http://example.com/search?id=' (or press Enter to use http://{target_ip}/search?id=): {reset}")
        if target_url_input.strip() == "":
            target_url = default_url
        else:
            target_url = target_url_input
        print(f"{yellow}[*] SQL Injection scanning has started. Requests will be sent every 2 seconds.{reset}")
        for payload in payload_list:
            try:
                response = requests.get(f"{target_url}{payload}")
                if "error" in response.text.lower() or "syntax" in response.text.lower():
                    print(f"{green}[+] SQL Injection vulnerability found! Manual verification is recommended. => {payload}{reset}")
                    with open("Log/sql_log.txt", "a") as file:
                        file.write(f"[+] SQL Injection vulnerability found! Manual verification is recommended. => {payload}")
            except requests.exceptions.RequestException as e:
                print(f"{red}[!] An error occurred while sending the request: {reset}{e}")
            time.sleep(2)
    elif mod == 2:
        default_url = f"https://{target_ip}/search?id="
